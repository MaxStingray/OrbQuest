...
namespace ooparts.dungen
{
	...
	public class Map : MonoBehaviour
	{
		...
		[HideInInspector] public int RoomCount; //final number of rooms???
		public RoomSetting[] RoomSettings; //array of each pair of room/wall materials to be applied to our rooms
		...
		public IEnumerator Generate()
		{
			...
			{
				_tilesTypes = new TileType[MapSize.x, MapSize.z];//general reference to the overall map???
				_rooms = new List<Room>();//new list of rooms

				// Generate Rooms
				for (int i = 0; i < RoomCount; i++)//loop through the room count we are aiming for set in inspector
				{
					Room roomInstance = CreateRoom();//create room via return method and store in roomInstance variable
					if (roomInstance == null)//check to see if we reached the maximum or some other error
					{
					...
					roomInstance.Setting = RoomSettings[Random.Range(0, RoomSettings.Length)];//assign the room size from parameters 
					StartCoroutine(roomInstance.Generate());//start generating a room inside of the room class

                    // Generate Player or Monster
                    if (_hasPlayer)
                    {
                        yield return roomInstance.CreateMonsters();
                        yield return roomInstance.CreatePickups();
                    }
                    else
                    {
                ...
                int whichRoom = Random.Range(1, _rooms.Count);//pick a random room excluding the first to contain stairs
                if (RoomMapManager._currFloor < RoomMapManager._numFloors)
                {
                    Debug.Log("room " + _rooms[whichRoom] + " is the exit");
                    Vector3 stairsPos = new Vector3(_rooms[whichRoom].transform.position.x, 0.0f, _rooms[whichRoom].transform.position.z);
                    GameObject stairs = Instantiate(stairsPrefab, stairsPos, transform.localRotation);
                    stairs.transform.parent = _rooms[whichRoom].transform;
                    _rooms[whichRoom].gameObject.tag = "StairsRoom";
                    Destroy(_rooms[whichRoom]._monstersObject);
                    Destroy(_rooms[whichRoom]._pickupObject);
                }
                else if (RoomMapManager._currFloor >= RoomMapManager._numFloors)
                {
                    Debug.Log("room " + _rooms[whichRoom] + " is the boss' lair");
                    Vector3 bossPosPos = new Vector3(_rooms[whichRoom].transform.position.x, 0.5f, _rooms[whichRoom].transform.position.z);
                    GameObject boss = Instantiate(bossPrefab, bossPosPos, transform.localRotation);
                    boss.gameObject.tag = "bossMonster";
                    boss.transform.parent = _rooms[whichRoom].transform;
                    _rooms[whichRoom].gameObject.tag = "BossRoom";
                }
                ...
                RoomMapManager.generated = true;
			}

            pathfinder.Scan();//tell the pathfinder to find new walkable areas in map
			stopwatch.Stop();
